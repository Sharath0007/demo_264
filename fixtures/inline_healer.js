...
function _truncateHealLogOnce() {
  ...
  try {
    ...
    // Non-fatal — if we can't truncate, we'll just append to the old log
...
async function heal(page, stepLabel, action, value, locatorFn) {
  ...
  if (originalLocator) {
    try {
      ...
      console.log(`[inline-heal] step "${stepLabel}" — original failed, attempting heal...`);
      ...
      try {
        ...
        console.log(`[inline-heal] step "${stepLabel}" — OpenAI failed: ${aiError.message}`);
        _appendHealLog({
      ...
      try {
        ...
        console.log(`[inline-heal] step "${stepLabel}" — eval failed: ${evalError.message}`);
        _appendHealLog({
          timestamp: new Date().toISOString(),
          spec_file: getCallingSpecFile(),
          step: stepLabel, action,
      ...
      try {
        ...
        console.log(`[inline-heal] step "${stepLabel}" — healed locator also failed`);
        _appendHealLog({
          timestamp: new Date().toISOString(),
          spec_file: getCallingSpecFile(),
          step: stepLabel, action,
      ...
      console.log(`[inline-heal] step "${stepLabel}" — HEALED: ${aiResponse.description || aiResponse.locator_code}`);
      _appendHealLog({
        timestamp: new Date().toISOString(),
        spec_file: getCallingSpecFile(),
        step: stepLabel, action,
